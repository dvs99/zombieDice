	...
	abstract class Room
	{
		...
		*/
		...
		private void receiveAndProcessNetworkMessagesFromMember(TcpMessageChannel pMember)
		{
			...
			{
				ASerializable message = pMember.ReceiveMessage();
				if (message is Hb)
					_server.Beat(pMember);
				handleNetworkMessage(message, pMember);
			}
		}
		...
		/**
		 * Sends a message to all members in the room except for the ones in the list.
		 */
		protected void sendToAllBut(ASerializable pMessage, List<TcpMessageChannel> usersNotToSend)
		{
			foreach (TcpMessageChannel member in _members)
			{
				if (!usersNotToSend.Contains(member))
					member.SendMessage(pMessage);
			}
		}
